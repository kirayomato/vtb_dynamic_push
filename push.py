...
class Push(object):
    ...
    def push_for_bili_dynamic(self, uname=None, dynamic_id=None, content=None, pic_url=None, dynamic_type=None, dynamic_time=None):
        ...
        if uname is None or dynamic_id is None or content is None:
            logger.error('【推送】缺少参数，uname:[{}]，dynamic_id:[{}]，content:[{}]'.format(
                uname, dynamic_id, content[:30]))
            return
        ...
        if dynamic_type == 1:
            ...
            title_msg = '投稿了'
        title = '【{uname}】{dynamic_type}'.format(
            uname=uname, dynamic_type=title_msg)
        content = '{content}[{dynamic_time}]'.format(
            content=content[:100] + (content[100:] and '...'), dynamic_time=dynamic_time)
    ...
    def push_for_weibo_dynamic(self, username=None, mblog_id=None, content=None, pic_url=None, jump_url=None, dynamic_time=None):
        ...
        if username is None or mblog_id is None or content is None:
            logger.error('【推送】缺少参数，username:[{}]，mblog_id:[{}]，content:[{}]'.format(
                username, mblog_id, content[:30]))
            return
        title_msg = '发微博了'
        title = '【{username}】{title_msg}'.format(
            username=username, title_msg=title_msg)
        content = '{content}[{dynamic_time}]'.format(
            content=content[:100] + (content[100:] and '...'), dynamic_time=dynamic_time)
    ...
    def _server_chan_push(self, title, content, url=None):
        ...
        response = util.requests_post(push_url, '推送_serverChan', params={
    ...
    def _server_chan_turbo_push(self, title, content, url=None):
        ...
        response = util.requests_post(push_url, '推送_serverChan_Turbo', params={
